...
# Logging
log_file = open("training_log_C4.txt", "w", encoding="utf-8")

# Transform (light augment assumed default)
...
# Replace classifier (FC only)
num_classes = len(train_data.classes)
model.fc = nn.Linear(model.fc.in_features, num_classes)
model = model.to(device)

# Loss and optimizer (different optimizer = SGD)
...
for epoch in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        early_stop_counter = 0
        torch.save(model.state_dict(), 'car_classifier_resnet50_C4_best.pth')
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Training vs Validation Loss (C4)")
plt.legend()
plt.grid()
plt.savefig("loss_curve_C4.png")
plt.show()

# Evaluation
model.load_state_dict(torch.load('car_classifier_resnet50_C4_best.pth'))
model.eval()
...
print("\n📋 Classification Report (C4):\n", report)

with open("classification_report_C4.txt", "w") as f:
    f.write(report)
...
plt.ylabel("Actual")
plt.title("Confusion Matrix (C4)")
plt.savefig("confusion_matrix_C4.png")
plt.show()
