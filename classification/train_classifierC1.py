# === CONFIG ID: C1 - ResNet50 | FC only + light augment ===
...
# Logging
log_file = open("training_log_C1.txt", "w", encoding="utf-8")

# Transforms (Light augment for train, standard for val)
...
# Replace classifier (FC only training)
...
for epoch in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        early_stop_counter = 0
        torch.save(model.state_dict(), 'car_classifier_C1_best.pth')
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Training vs Validation Loss (C1)")
plt.legend()
plt.grid()
plt.savefig("loss_curve_C1.png")
plt.show()

# Evaluation
model.load_state_dict(torch.load('car_classifier_C1_best.pth'))
model.eval()
...
print("\n📋 Classification Report (C1):\n", report)

with open("classification_report_C1.txt", "w") as f:
    f.write(report)
...
plt.ylabel("Actual")
plt.title("Confusion Matrix (C1)")
plt.savefig("confusion_matrix_C1.png")
plt.show()
